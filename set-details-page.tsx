...
export function SetDetailsPage({ labId, labNumber, setNumber, manufacture, onBack, onBackToDashboard, userName, onLogout, onNavigateToLabs, isDemoMode }: SetDetailsPageProps) {
  ...
  const handleRetrieveLogs = async () => {
    ...
    try {
      ...
      // Make API call
      const response = await fetch(LOG_COLLECTION_ENDPOINT, {
        method: "POST",
        headers: {
  ...
  const DEVICE_INVENTORY_ENDPOINT = `${API_BASE_URL}/device-inventory/get_all_lls_inventory`;
  const LOG_COLLECTION_ENDPOINT = `${API_BASE_URL}/log-collector/start_log_collection`;
  ...
  const fetchDeviceInventory = async () => {
    ...
    try {
      ...
      const url = `${DEVICE_INVENTORY_ENDPOINT}?lab_id=${encodeURIComponent(labId)}`;
