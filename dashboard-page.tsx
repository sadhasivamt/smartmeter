...
export function DashboardPage({ onLogout, userName, onNavigateToLabs, isDemoMode }: DashboardPageProps) {
  ...
  const LOG_COLLECTIONS_ENDPOINT = `${API_BASE_URL}/api/log-collections`;
  ...
  const fetchLogCollections = async (nextPageKey: NextPageKey | null = null, showToast: boolean = false) => {
    ...
    if (isDemoMode) {
      setTimeout(() => {
        ...
        const manufacturers = ["Toshiba 4G", "VMO2", "EDMI", "WNC"];
        const allMockRecords = Array.from({ length: 20 }, (_, i) => {
          const statusCode = [105, 106, 107, 108][i % 4];
          const statusText = statusCode === 108 ? "Ready" : "Processing";
          const cabinetNumber = (i % 10) + 1;
          return {
            lab_id: `lab-${(i % 3) + 1}`,
            cabinet_id: `a${String(cabinetNumber).padStart(2, '0')}`,
            ...
            start_time: `2025-01-0${(i % 7) + 1}T${String((i % 12) + 8).padStart(2, '0')}:00:00+0000`,
            end_time: `2025-01-0${(i % 7) + 1}T${String((i % 12) + 10).padStart(2, '0')}:00:00+0000`,
            submitted_time: `2025-01-0${(i % 7) + 1}T${String((i % 12) + 7).padStart(2, '0')}:00:00+0000`,
        ...
        // Sort by submitted_time in descending order (most recent first)
        combinedRecords.sort((a, b) => {
          const dateA = new Date(a.submitted_time).getTime();
          const dateB = new Date(b.submitted_time).getTime();
        ...
        const mockData: LogCollectionsResponse = {
          next_page_key: currentPage < Math.ceil(combinedRecords.length / DEFAULT_LIMIT) ? { id: "mock-next-key", start_time: "2025-01-08T10:30:00+0000" } : null,
        ...
        const rows: TableRow[] = mockData.log_collections.map((item, index) => ({
          ...
          setDetails: `${item.lab_id} / ${item.cabinet_id} / ${item.manufacture}`,
          timeSubmitted: formatDateTime(item.submitted_time),
    ...
    setTimeout(() => {
      ...
      const mockData: LogCollectionsResponse = {
        ...
        log_collections: Array.from({ length: DEFAULT_LIMIT }, (_, i) => {
          ...
          return {
            lab_id: `galaxy${(i + offset) % 5 + 1}`,
            cabinet_id: `a${String((i + offset) % 10 + 1).padStart(2, '0')}`,
            manufacture: `manufacturer-${(i + offset) % 3 + 1}`,
            transaction_id: `txn-${Math.random().toString(36).substr(2, 9)}-${i + offset}`,
            log_collection_status: ["log_collection_started", "log_collection_in_progress", "log_collection_completed", "log_collection_cancelled"][i % 4],
            log_collection_status_code: [105, 106, 107, 108, 109][i % 5],
            start_time: new Date(Date.now() - (i + offset) * 3600000).toISOString(),
            end_time: new Date(Date.now() - (i + offset) * 1800000).toISOString(),
            submitted_time: new Date(Date.now() - (i + offset) * 7200000).toISOString(),
            task_desc: `Task description for item ${i + offset + 1}`,
          };
        }),
      };
    ...
    try {
      ...
      const response = await fetch(LOG_COLLECTIONS_ENDPOINT, {
        method: "POST",
        headers: {
  ...
  const processApiResponse = (data: LogCollectionsResponse) => {
    ...
    const mappedData: TableRow[] = data.log_collections.map((log, index) => ({
      ...
      setDetails: `${log.lab_id} / ${log.cabinet_id} / ${log.manufacture}`,
      timeSubmitted: log.submitted_time,
